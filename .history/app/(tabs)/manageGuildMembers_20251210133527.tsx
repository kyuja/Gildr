...
export default function manageGuildMembers() {
    ...
    <KeyboardAvoidingView style={{flex :1}} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}
        ...
        <View style={{backgroundColor:'white',borderRadius:10,padding:10,marginTop:10, height:300}}>
            ...
            <ScrollView>
                {members.map( (member) => (
                    <View key={member.id} style={{padding:15, backgroundColor:isMarkedForDeletion(member.id) ? 'gray' : 'white'}}>
                    <Text style={{fontWeight: 'bold', color: isMarkedForDeletion(member.id) ? 'gray' : 'black'}}> {member.name}</Text>
                    <Pressable onPress={() => toggleMemberDelete(member.id)}>
                        <Ionicons name="remove-circle" size={40} style={{position:'absolute', right:10,padding:5, color:isMarkedForDeletion(member.id) ? 'maroon' : 'black'}}></Ionicons>
                    </Pressable>
                    </View>
                ))}
            </ScrollView>
        ...
        <View style={{flexDirection:'row', justifyContent:'space-between', marginTop:20 }}>
            <Pressable style={styles.saveButton} onPress= {deleteMember}>
