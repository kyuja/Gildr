        ...
        <View style={{backgroundColor:'white',borderRadius:10,padding:10,marginTop:10, height:300}}>
            ...
            <ScrollView>
                {memberList.map( (member) => (
                    <View key={member.id} style={{padding:15}}>
                    ...
                    <Pressable onPress={() => toggleMemberDelete(member.id)}>
                        <Ionicons name="remove-circle" size={40} style={{alignSelf:'flex-end', right:10,padding:5, color:isMarkedForDeletion(member.id) ? 'maroon' : 'black'}}></Ionicons>
                    </Pressable>
                    </View>
                ))}
            </ScrollView>
